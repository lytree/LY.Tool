...
public class MinMaxCloseControl : TemplatedControl
{
    ...
    private void OwnerWindowOpened(object sender, EventArgs args)
    {
        _windowStateObservable = new FACompositeDisposable(
            _owner.GetObservable(Window.WindowStateProperty).FASubscribe(OnWindowStateChanged),
            _owner.GetObservable(WindowBase.IsActiveProperty).FASubscribe(OnWindowActiveChanged));
    }
