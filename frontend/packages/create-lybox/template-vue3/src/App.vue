...
interface DemoCommands extends Record<string, RpcMethodDefinition> {
  ...
  GetPluginInfoAsync: { args: []; result: { id: string; name: string; version: string } };
...
async function callGreet(): Promise<void> {
  ...
  try {
    ...
    const prefix = await demo.invoke('GreetAsync', greetName.value);
...
async function callAdd(): Promise<void> {
  ...
  try {
    ...
    addResult.value = await demo.invoke('AddAsync', addA.value, addB.value);
  } catch (err) {
...
async function callGetInfo(): Promise<void> {
  ...
  try {
    ...
    pluginInfo.value = await demo.invoke('GetPluginInfoAsync');
  } catch (err) {
  ...
  <div class="lybox-app">
    ...
    <header class="lybox-app__header">
      <div class="lybox-app__title">
        ...
        <span class="ly-badge" :class="webViewMode ? 'ly-badge-primary' : 'ly-badge-success'">
          {{ webViewMode ? 'WebView IPC' : 'Browser (Mock)' }}
        </span>
      </div>
      <button class="ly-btn ly-btn-secondary" @click="toggleTheme">
    ...
    <main class="lybox-app__main">
      <!-- RPC 调用区 -->
      <section class="ly-card">
        <h2 class="lybox-section-title">RPC 调用</h2>
        ...
        <div class="lybox-row">
          ...
          <input
            v-model="greetName"
            class="ly-input input-narrow"
            type="text"
            placeholder="输入名字"
            @keyup.enter="callGreet"
          />
          <button class="ly-btn ly-btn-primary" :disabled="greetLoading" @click="callGreet">
            {{ greetLoading ? '调用中...' : '调用' }}
          </button>
        </div>
        ...
        <div class="lybox-row">
          ...
          <input v-model.number="addA" class="ly-input input-narrow" type="number" />
          <span class="lybox-row__operator">+</span>
          <input v-model.number="addB" class="ly-input input-narrow" type="number" />
          <button class="ly-btn ly-btn-primary" :disabled="addLoading" @click="callAdd">
            {{ addLoading ? '调用中...' : '调用' }}
          </button>
        </div>
        ...
        <div class="lybox-row">
          ...
          <button class="ly-btn ly-btn-primary" :disabled="infoLoading" @click="callGetInfo">
      ...
      <!-- SSE 事件区 -->
      <section class="ly-card">
        <h2 class="lybox-section-title">SSE 事件订阅（tick）</h2>
        <div class="lybox-row">
    ...
    <footer class="lybox-app__footer">
      <button class="ly-btn ly-btn-secondary" @click="toggleDebugPanel">
...
<style scoped>
/* 布局与辅助样式：仅保留 SDK 组件类（.ly-btn/.ly-card/.ly-badge/.ly-input 等）未覆盖的部分。
   组件样式统一由 @lytree/sdk/components 提供。颜色均引用 @lytree/sdk/css 的 CSS 变量，
   不硬编码，与宿主 Avalonia Fluent 主题一致。 */
...
/* 窄输入框（配合 .ly-input 使用，SDK 的 .ly-input 为通栏宽） */
.input-narrow {
  width: 80px;
}
