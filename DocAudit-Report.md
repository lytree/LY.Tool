...
> **修订状态（2026-08-01）**：P0 / P1 / P2 中**文档侧**的修正已全部应用（README.md + AGENTS.md）：Cake 版本、测试/CI 描述、插件数 10→12、src 项目补全、启动流程 3 阶段、构建目标补全、命名统一为 Cake.Sdk、新增前端 monorepo 小节、补入两份 WebView 文档索引、产物树补 `bin/tools/`。**仅剩代码侧**遗留：`build.cs` 的 `fluent-window` 目标仍引用已删除的 `src/LYBox.Layout.Fluent`（无害死代码，文档已注明「如不存在则跳过」），如需可单独清理。
...
2. **两份 WebView 相关文档成为「孤儿文档」**
   - `docs/WebView-IPC-Guide.md`（44KB）、`docs/LYBox-MockServer-Guide.md`（17KB）已在仓库中，但 **`README.md` 的「详细文档」表与 `AGENTS.md` 均未引用它们**，新读者无法发现。
   - 这两份文档恰好对应缺失的 `BlazorApp` / `WebTemplate` 插件与 `tools/LYBox.MockServer`，应补入文档索引。

3. **`tools/LYBox.MockServer`（lybox-mock dotnet tool）未被完整记录**
   - `build/build.cs` 有独立的 `tool` 构建目标（`PackTool`）用于打包该 dotnet tool。
   - `AGENTS.md` 仅在参数列表顺带提及；`README.md` 的「构建与运行」章节 **未列出 `--build=tool` 目标**。

4. **构建目标清单不全**
   - 两文档均未记录以下实际支持的参数：`--build=fluent-window`、`--build=tool`、`--host-version`（README 未列）、`--plugin-version`、`--plugin=<Name>`、`--nuget-source`。
   - `AGENTS.md` 记录了 `--host-version` / `--package-version`，但两文档都漏了 `fluent-window` / `tool` / `plugin-version` / `plugin` / `nuget-source`。
...
- `build/build.cs` 的 `fluent-window` 构建目标引用 `src/LYBox.Layout.Fluent/LYBox.Layout.Fluent.csproj`，但 **该项目已被删除**（git 记录 `6f05b21 refactor: Remove Fluent UI components and related services`）。`build.cs` 用 `File.Exists` 守卫，故仅为无害死代码，但 `fluent-window` / `FluentWindow` 目标现在已无意义，建议清理或在文档中标注「已废弃」。

---

## 四、文档内部矛盾（README 与 AGENTS 互相不一致）

| # | 矛盾点 | README | AGENTS |
|---|--------|--------|--------|
| 1 | 构建系统命名 | "Cake Frosting" | "Cake Frosting（…Cake.Sdk 6.2.0）"，而 `build.cs` 注释明确「符合官方 Cake.Sdk 项目设置模式，不再继承 FrostingContext」 | 建议统一为 **Cake.Sdk** |
| 2 | `--build=all` 含义 | "bin（启动器 + NuGet）+ plugin" | 同上 | 实际 `All = Bin | FluentWindow | Plugin | Tool`，两文档都少写了 FluentWindow 与 Tool |
| 3 | 应用启动流程 | 3 阶段（准确） | `LoadPluginsAsync()` 单阶段（过时） | 以 README / 代码为准，更新 AGENTS |
| 4 | Cake 版本 | 6.1.0（错） | 6.2.0（对） | 统一为 6.2.0 |
...
- [ ] 在 `README.md` 与 `AGENTS.md` 文档索引中补入 `docs/WebView-IPC-Guide.md`、`docs/LYBox-MockServer-Guide.md`。
- [ ] 补全构建目标说明：`--build=fluent-window`、`--build=tool`、`--host-version`、`--plugin-version`、`--plugin`、`--nuget-source`。
- [ ] 说明 `tools/LYBox.MockServer`（lybox-mock）的用途与 `--build=tool` 打包方式。
- [ ] 说明 `LYBox.Plugin.Shared.Chart` / `Shared.ProDataGrid` 与对应插件的关系。

**P2（一致性 / 死代码清理）**
- [ ] 统一构建系统命名为「Cake.Sdk」。
- [ ] 修正 `--build=all` 说明为 `Bin | FluentWindow | Plugin | Tool`。
- [ ] 评估删除 `build.cs` 中已失效的 `fluent-window` / `FluentWindow` 目标（或显式标注废弃）。

---

## 七、总结

文档目前最大的问题是 **「前端 / WebView 化」重构未被文档吸收**：新增的 `frontend/` monorepo、两个 WebView 插件（`BlazorApp`/`WebTemplate`）、`MockServer` 工具、相关共享项目与两份 WebView 文档，在 `README.md` 与 `AGENTS.md` 中几乎完全缺席；同时插件数量（10→12）、src 项目（5→9）、测试/CI 的存在性、Cake 版本、启动流程等均有事实错误。建议按 P0→P1→P2 顺序修订，并将两份 WebView 文档纳入索引以消灭孤儿文档。
