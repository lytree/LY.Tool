...
4. **构建目标清单不全**
   - 两文档均未记录以下实际支持的参数：`--build=tool`、`--host-version`（README 未列）、`--plugin-version`、`--plugin=<Name>`、`--nuget-source`。
   - `AGENTS.md` 记录了 `--host-version` / `--package-version`，但两文档都漏了 `tool` / `plugin-version` / `plugin` / `nuget-source`。
...
| 2 | `--build=all` 含义 | "bin（启动器 + NuGet）+ plugin" | 同上 | 实际 `All = Bin | Plugin | Tool`，两文档都少写了 Tool |
